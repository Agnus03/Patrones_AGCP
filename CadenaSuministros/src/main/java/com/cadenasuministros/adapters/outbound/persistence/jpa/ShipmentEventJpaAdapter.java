...
@Component
public class ShipmentEventJpaAdapter extends AbstractJpaAdapter<ShipmentEvent, ShipmentEventJpaEntity>
        implements ShipmentEventRepository {

    private final SpringDataShipmentEventRepository eventRepo;
    ...
    @Override
    protected ShipmentEventJpaEntity toEntity(ShipmentEvent d) {
    ...
    @Override
    protected ShipmentEvent toDomain(ShipmentEventJpaEntity e) {
    ...
    public List<ShipmentEvent> findByShipmentIdOrderByTimestampDesc(UUID shipmentId) {
        return eventRepo.findByShipmentIdOrderByTimestampDesc(shipmentId).stream()
                .map(this::toDomain)
